...
<%
	...
	// 객체가 생성되는 시점에 /upload 폴더에 자동 파일이 저장된다.
		//생성자 MultipartRequest() 의 인자로 5 개(request, realPath, sizeLimit, "utf-8",new DefaultFileRenamePolicy()) 전달
	MultipartRequest mr= new MultipartRequest(request,
			realPath,	// 파일 저장 경로
			sizeLimit,	// 사이즈 제한
			"utf-8",	// 파일명 한글 처리
			new DefaultFileRenamePolicy());	// 파일명 겹치지 않도록 (파일명 같으면 1, 2, 3 붙여줌)
	// 업로드된 파일에 대한 정보는 mr 객체를 통해서 얻어온다.
	String writer= mr.getParameter("writer");
	String title= mr.getParameter("title");
		//insertform.jsp 에서 name, id 를 myFile 로 했으므로 
